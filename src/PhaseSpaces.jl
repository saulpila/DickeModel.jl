...
export Q_of_θφ,P_of_θφ,θ_of_QP,φ_of_QP,jz,jx,jy,arc_between_θφ,arc_between_QP
    """
    ```julia
    function Q_of_θφ(θ,φ)
    ```
    Returns
    ```math
    ...
    ```
    """
    Q_of_θφ(θ::Real,φ::Real)=sqrt(2*(1-cos(θ)))*cos(φ)
    """
    ```julia
    function P_of_θφ(θ,φ)
    ```
    Returns
    ```math
    ...
    ```
    """
    P_of_θφ(θ::Real,φ::Real)=-sqrt(2*(1-cos(θ)))*sin(φ)
    """
    ```julia
    ...
    Returns
    ```math
        \\theta =- \\arccos(1 - \\frac{Q^2 + P^2}{2})}.
    ```
    """
    θ_of_QP(Q::Real,P::Real)=acos(1-(Q^2+P^2)/2)
    """
    ```julia
    function φ_of_QP(Q,P)
    ```
    Returns
    ```math
    ...
    """
    φ_of_QP(Q::Real,P::Real)=mod(atan(-P,Q),2*pi)
    """
    ```julia
    function jz(Q,P)
    ```
    ...
    """
    ```julia
    function arc_between_θφ(θ1,φ1,θ2,φ2)
    ```
    Returns 
    ```math
    ...
    ```
    """
    arc_between_θφ(θ1::Real,φ1::Real,θ2::Real,φ2::Real)=acos(clamp(cos(θ1)*cos(θ2)+ sin(θ1)*sin(θ2)*cos(φ1 - φ2),-1,1))
    """
    ```julia
    ...
    ```
    """
    arc_between_QP(Q1::Real,P1::Real,Q2::Real,P2::Real)=arc_between_θφ(θ_of_QP(Q1,P1),φ_of_QP(Q1,P1),θ_of_QP(Q2,P2),φ_of_QP(Q2,P2))
end
